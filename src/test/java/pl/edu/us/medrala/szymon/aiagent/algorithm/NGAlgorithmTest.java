...
class NGAlgorithmTest {

    private NGAlgorithm algorithm;
    ...
    private Card noHeroCard;
    @BeforeEach
    void setup() {
        algorithm = new NGAlgorithm();
        ...
        spyCardMedium = new Card(303L, "Spy Medium", 7, 7,
                Ability.SPY, Type.NONE, 1, Range.CLOSE_COMBAT,
                Fraction.SCOIATAEL, new byte[0]);
        noHeroCard = new Card(303L, "No Hero", 7, 7,
                Ability.NONE, Type.NONE, 1, Range.CLOSE_COMBAT,
                Fraction.SCOIATAEL, new byte[0]);
    ...
    @Test
    void shouldExecuteAndReturnWeaknessSpyCardWhenMultipleSpyCardsAvailableWhenPhaseFirstAndRoundFirst() {
        yourBoard.getHand().remove(ciriCard);
        yourBoard.getHand().remove(geraltCard);
        ...
        Assertions.assertTrue(card.isPresent());
        Assertions.assertEquals(spyCardMedium, card.get());
    }
    ...
    void shouldExecuteAndReturnSpyCardWhenPhaseFirstAndRoundSecond() {
        ...
        Assertions.assertTrue(card.isPresent());
            Assertions.assertEquals(getCardWithAbility(Ability.SPY), card.get());
    }

    @Test
    void shouldExecuteAndReturnWeaknessSpyCardWhenMultipleSpyCardsAvailableWhenPhaseFirstAndRoundSecond() {
        yourBoard.isSecondRound();
        yourBoard.getHand().remove(ciriCard);
        yourBoard.getHand().remove(geraltCard);
        yourBoard.getHand().add(spyCardMedium);
